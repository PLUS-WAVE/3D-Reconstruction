...
void Window::Release()
{
	...
	clbkOpenScene = fastdelegate::FastDelegate<bool(LPCTSTR, LPCTSTR)>();
	clbkExportScene = fastdelegate::FastDelegate<bool(LPCTSTR, LPCTSTR, bool)>();
	clbkRayScene = fastdelegate::FastDelegate<void(const SEACAVE::Ray3&, int)>();
	clbkCompilePointCloud = fastdelegate::FastDelegate<void()>();
	clbkCompileMesh = fastdelegate::FastDelegate<void()>();
}
