...
)
{
	...
	{
		std::cerr << std::endl << "输入的SfM_Data文件 \"" << SFMDataFilename << "\" 无法读取." << std::endl;
		return EXIT_FAILURE;
	}
	...
	if (!regions_provider->load(sfm_data, OutputPath, regions_type))
	{
	...
	else
	{
		...
		switch (ePairmode)
		{
		case PAIR_EXHAUSTIVE: std::cout << "穷举匹配" << std::endl;
			break;
		case PAIR_CONTIGUOUS: std::cout << "序列匹配" << std::endl;
			break;
		case PAIR_FROM_FILE: std::cout << "用户自定义的匹配" << std::endl;
			break;
		}
		...
		system::Timer timer;
		{
			...
			switch (ePairmode)
			{
			case PAIR_EXHAUSTIVE: pairs = exhaustivePairs(sfm_data.GetViews().size());
				break;
			case PAIR_CONTIGUOUS: pairs = contiguousWithOverlap(sfm_data.GetViews().size(), iMatchingVideoMode);
				break;
			case PAIR_FROM_FILE:
		...
		std::cout << "Putative Match完成，用时(秒): " << timer.elapsed() << std::endl << std::endl;
	}

	std::cout << "-- 进行几何模型的估计过滤 --" << std::endl;
	...
	if (filter_ptr)
	{
		...
		case ESSENTIAL_MATRIX:
		{
			...
			{
				const size_t putativePhotometricCount = map_PutativesMatches.find(pairwisematches_it.first)->second.
				                                                             size();
		...
		std::cout << "几何模型的估计过滤完成，用时(秒): " << timer.elapsed() << std::endl;
